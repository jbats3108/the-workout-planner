...
- Backend quality: `composer quality-fix` (Rector→Pint×2) / `composer quality` (fix then PHPStan) / `composer analyse` / `composer refactor` / `composer refactor:dry`. CI applies fixes (Rector→Pint×2, ESLint→Prettier) and auto-commits; PHPStan and `vue-tsc` are advisory (`continue-on-error`). Larastan level 7 with `parseModelCastsMethod: true`. Rector skips coding-style rules so Pint owns formatting.
...
Path-scoped app conventions (Spatie Data, FE domain modules) live in `.cursor/rules/` and load when matching files are in context — not restated here. Long-form: `docs/adr/0001-…`, `docs/adr/0005-…`.

...
- **Since-last-deload counts** — per routine on dashboard (finished normals since that routine’s latest finished deload); soft Deload hint at ≥ **Deload Velocity** (`deload_every_n`, editor Deload settings; 0 = never); no shared cycle counter

...
- Larger shared exercise catalog: default `database/data/exercises.json` (~729 lifts: free-exercise-db + original short-name set). Import with `php artisan exercises:import`. Audit orphans / missing equipment with `php artisan exercises:audit` (add `--json` on prod).
...
### Non-obvious notes
- HTTP request/response boundaries use **Spatie Laravel Data** under `App\{Domain}\Data\...` (see `docs/adr/0001-spatie-laravel-data-http-boundaries.md`). Prefer those over `$request->validate()` or ad-hoc Inertia prop arrays. DTO constructor props are always `public readonly`; prefer PHP types over redundant `#[Present]` / `#[ArrayType]` (omit empty optional lists; attributes only for constraints types cannot express).
- Frontend mirrors PHP domains under `resources/js/{domain}/` (types, lib, composables, components); Inertia pages in `pages/` stay thin. See `docs/adr/0005-frontend-domain-modules.md`.
